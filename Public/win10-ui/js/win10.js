...
window.Win10 = {
    ...
    _init:function () {   
         ...
         * 设置语言start
         +----------------------------------------------------------------------------------------------------------------------------------------------------
         */
        Win10._lang=(navigator.language || navigator.browserLanguage).toLowerCase();
        /**
         +----------------------------------------------------------------------------------------------------------------------------------------------------
         * 设置语言End
         +----------------------------------------------------------------------------------------------------------------------------------------------------
         ...
         */
        /*1.设置默认右击桌面菜单*/
        Win10._wythe_deskList = [
            ['<i class="fa fa-fw fa-window-maximize"></i> '+Win10.lang('添加桌面','Add desk'),function () {
                layer.open({
                    type:2,
                    title:Win10.lang('增加外部桌面','Add Desk Icon'),
                    content:$home+'/Index/addExternalDesk',
                    btn:[Win10.lang('确认','Confirm'),Win10.lang('取消','Cancel')],
                    area:['500px','300px'],
                    btn1:function(index,layero){
                        var FormValue = layero.find('iframe').contents().find('#deskForm').serializeArray();
                        $.ajax({
                            url:$home+'/Index/insertExternalDesk',
                            type:'post',
                            data:FormValue,
            ...
            ['<i class="fa fa-fw fa-window-maximize"></i> '+Win10.lang('进入全屏','Enable Full Screen'),function (res) {Win10.enableFullScreen()}],
            ['<i class="fa fa-fw fa-window-restore"></i> '+Win10.lang('退出全屏','Disable Full Screen'),function (res) {Win10.disableFullScreen()}],
            '|',
            ['<i class="fa fa-fw fa-info-circle"></i> '+Win10.lang('刷新','refresh'),function (res) {
                window.location.reload();
            }],
/*            ['<i class="fa fa-fw fa-info-circle"></i> '+Win10.lang('关于','About Us'),function (res) {
                layer.msg('');
            }],*/
        ];
        /*2.设置右击桌面图标的菜单*/
        Win10._wythe_deskList_icon = [['<i class="fa fa-fw fa-window-maximize"></i> '+Win10.lang('删除','Delete'),function (e) {
           $.ajax({
            url:$home+'/Index/deleteDesk',
            type:'post',
            data:{id:$(e.data).data('id')},
            success:function(res){
                layer.msg(res.msg);
                if(res.status){
                    Win10.deleteDesk(e.data);
                }
            }
           });
        }]];
        /*3.设置菜单右击菜单*/
        Win10._wythe_menuList = [['<i class="fa fa-fw fa-window-maximize"></i> '+Win10.lang('加入桌面','Insert to Desk'),function (e) {  
            $.ajax({
                url:$home+'/Index/addDesk',
                type:'post',
                data:{id:$(e.data).data('id')},
                dataType:'json',
                success:function(res){
        ...
        Win10.setContextMenu('#win10>.desktop',Win10._wythe_deskList);
        Win10.setContextMenu('#win10>.desktop>#win10-shortcuts>.shortcut',Win10._wythe_deskList_icon);
        Win10.setContextMenu('#win10>#win10-menu>.list>.sub-item',Win10._wythe_menuList);
        Win10.setContextMenu('#win10_btn_group_middle',Win10._wythe_taskBar);      
...
/*        $(document).on('click', '.win10-btn-change-url', function () {
        ...
        $(document).on('click', '.win10-btn-toolbar', function () {
            var index = $(this).attr('index');
            var toolbar = Win10.getLayeroByIndex(index).find('iframe').contents().find('#toolbar');
            toolbar.slideToggle(700,function(){
                 Win10.getLayeroByIndex(index).find('iframe').contents().find('#toolbar').find('#WindowResize').click();
            });
    ...
    openUrl: function (url, title,areaAndOffset) {
        ...
        $('#win10>.desktop>#win10-shortcuts').on('mousedown','.shortcut',function(){
        ...
        layero_opened.find('.layui-layer-setwin').prepend(//'<a class="win10-btn-change-url" index="' + index + '" title="'+Win10.lang('修改地址','Change URL')+'" href="javascript:void(0)"><span class="fa fa-chain"></span></a>'+
                                                         '<a class="win10-btn-toolbar" index="' + index + '" title="'+Win10.lang('菜单','MENU')+'" href="javascript:void(0)"><span class="fa fa-align-justify"></span></a>'+
    ...
    exit:function () {
        layer.confirm(Win10.lang('确认要退出系统吗?','Are you sure you want to log out?'), {icon: 3, title:Win10.lang('提示','Prompt')}, function(index){
            document.body.onbeforeunload = function(){};
            window.location.href=$home+"/login/logout";
            window.close();
            layer.close(index);
            layer.alert(Win10.lang('关闭失败！系统异常！','close faild,System Error!'), {
    ...
    addDesk:function(deskData){
       var deskDom = '<div class="shortcut" onclick="Win10.openUrl(\''+deskData.url+'\',\'<i class=\\\''+deskData.iconcls+'\\\'></i>'+deskData.text+'\')" data-id="'+deskData.desk_id+'">'+
                        '<img class="icon" src="'+$public+'/win10-ui/img/icon/'+deskData.menu_img+'"/>'+
                        '<div class="title">'+deskData.text+'</div>'+
                    '</div> ' ;
       $('#win10>.desktop>#win10-shortcuts').append($(deskDom));  
       Win10.setContextMenu('#win10>.desktop>#win10-shortcuts>.shortcut',Win10._wythe_deskList_icon);//新增菜单重新绑定
       this.menuClose(); 
