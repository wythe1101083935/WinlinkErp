...
class ExportClientConfirmOrderController extends CommonController{
	/*
	 +----------------------------------------------------------
	 *首页展示页
	 +----------------------------------------------------------
	...
	*/
	public function XHRIndex(){
		...
		/*6.查询数据*/
			...
			$data = D('ViewClientConfirmOrder')->where($where)->page($postData['pageNumber'] ,$postData['pageSize'])->order($order)->select();
			$count = D('ViewClientConfirmOrder')->where($where)->count();
